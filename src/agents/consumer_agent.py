...
class ConsumerAgent(Agent):
    ...
    def __init__(self, unique_id, model, base_demand, price_sensitivity):
        ...
        self.demand_threshold_multiplier = model.config.get("consumer_demand_threshold_multiplier", 2.0)
    ...
    def step(self):
        ...
        # 3. Contribute to end-of-life pool (handled by model)
    
    def _calculate_demand(self):
        """Calculate demand based on current price elasticity."""
        # Demand formula: D = D_base * (P_current / P_initial)^elasticity
        # Elasticity is negative, so higher prices reduce demand
        
        price_ratio = self.model.current_price / self.model.initial_price
        
        # Use log for better numerical stability
        if price_ratio > 0:
        ...
        # Apply threshold: don't buy if price too high
        max_acceptable_price = self.model.initial_price * self.demand_threshold_multiplier
        if self.model.current_price > max_acceptable_price:
            self.current_demand *= 0.5  # Reduce demand by 50%
    ...
    def _purchase_from_retailers(self):
        ...
        # Try to purchase from retailers
        ...
        # Add fulfilled purchases to EOL pool (with product lifetime lag)
